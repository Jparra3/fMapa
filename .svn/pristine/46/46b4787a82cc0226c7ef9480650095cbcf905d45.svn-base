...
$retorno = array('exito' => 1, 'mensaje' => 'Se modificó la información correctamente', 'idTercero' => '', 'idClientePrincipal'=>null);
try {
    ...
    for ($i = 0; $i < count($arrSucursal); $i++) {
        ...
        if ($arrSucursal[$i]->idSucursal != null && $arrSucursal[$i]->idSucursal != "null" && $arrSucursal[$i]->idSucursal != "") {
            ...
            if($arrSucursal[$i]->principal != false && $arrSucursal[$i]->principal != "false" && $arrSucursal[$i]->principal != "FALSE"){
