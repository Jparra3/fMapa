...
session_start();
set_time_limit(0);
require_once '../../Seguridad/entidad/Empresa.php';
require_once '../../Seguridad/modelo/Empresa.php';
...
try {
    ...
    for ($i = 0; $i < count($arrInformacion); $i++) {
        ...
        if ($arrTercero != null) {
            ...
            $idEmpresa = validarExistenciaEmpresa($arrInformacion[$i]->empresa);
            
            if($idEmpresa == null || $idEmpresa == "" || $idEmpresa == "null"){
                throw new Exception("La empresa no existe en el aplicativo");
                return false;
            }
        ...
        //Se valida la existencia de la sucursal y cliente
        if ($arrTercero != null) {
            ...
            $clienteE->setLatitud(0);
            $clienteE->setLongitud(0);
...
/* * *********************** Validar existencia Empresa ********************** */
function validarExistenciaEmpresa($empresa) {
    
    $terceroE = new \entidad\Tercero();
    $terceroE->setTercero($empresa);
    
    $empresaE = new \entidad\Empresa();
    $empresaE->setTercero($terceroE);
    
    $empresaM = new \modelo\Empresa($empresaE);
    $arrEmpresa = $empresaM->consultar();

    if(count($arrEmpresa) > 0){
        return $arrEmpresa[0]["idEmpresa"];
    }else{
        return null;
    }
}
