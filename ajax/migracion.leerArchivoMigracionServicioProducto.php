    ...
    foreach (file($ruta) as $fila) {
        ...
        if ($contadorLineaArchivo >= 1) {//No se fija en el encabezado del archivo excel
            if($contenido[0] != "" && $contenido[0] != null && $contenido[0] != "null") {
                ...
                $arrProductoComposicion = explode("-", $contenido[1]);
                
                $arrDataMigracion[$contador]["codigoProductoComposicion"] = utf8_encode($arrProductoComposicion[0]);
                $arrDataMigracion[$contador]["productoComposicion"] = utf8_encode($arrProductoComposicion[1]);
                $arrDataMigracion[$contador]["cantidad"] = utf8_encode($contenido[2]);
                $contador++;
            }
        } else {
